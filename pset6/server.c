...
void method_header(char* method, const char* line);
void request_header(char* request, const char* line);
void version_header(char* version, const char* line);
int validate_method(const char* method);
int validate_request(const char* request); 
int validate_version(const char* version);
void separate_abs_path(char* abs_path, const char* request);
void separate_query(char* query, const char* request);
...
const char* valid_version = "HTTP/1.1";
...
char* indexes(const char* path)
{
    //to count the length of path
    int length = strlen(path);
    
    // to allocate memory on heap for the new path (we add 11 since index.html is the longest possible path that consists of 10 chars + null operator)
    BYTE* path2 = (char*) malloc(sizeof(char) * (length + 11));
    
    // to check whether index.php exists inside of path
    strcpy(path2, path);
    strcat(path2, "index.php");
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    // to initialize our variables - content and length
    *content = NULL;
    *length = 0;
    
    // to create a counter for bytes in a buffer
    int n;
    
    // to initialize a buffer of type char for 512 bytes
    BYTE buffer[BYTES];
    
    // a do - while loop to read data from the file into the buffer
    do {
        // to store the size of a buffer
        n = fread(buffer, 1, BYTES, file);
        
        // to add additional memory for *content with the new size
        *content = realloc(*content, *length + n);
        
        //to exit the function if *content is empty
        if (*content == NULL)       
            return false;
        
        // copy buffer contents into *content at appropriate position
        memcpy((*content + *length), buffer, n);
        
        // to update value of *length
        *length = *length + n;
    } while (n == BYTES);
...
const char* lookup(const char* path)
{
    if (path == NULL)
        return NULL;
    //by using strrchr function, we return a pointer to the last occurrence of '.' in path - store the ending(extension) of file with the dot operator to variable extension
    char* extension = strrchr(path, '.');
    //we convert the file extension to the MIME type by using strcasecmp function (since we also need to take into consideration the capital letters in the extension)    
...
{
    //to obtain the line length plus the null operator
    int linel = strlen(line) + 1;
    
    //to set the length of the http version string to be 25 (random choice for the number)
    int httpl = 25;
    
    //to allocate memory in heap for the required substrings of the request line string: method; request-target;http version
    
    BYTE* method = malloc(sizeof(char) * linel);
    BYTE* request = malloc(sizeof(char) * linel);
    BYTE* version = malloc(sizeof(char) * httpl);
    
    //to obtain the method, request and version headers from the line string. 
    
    method_header(method, line);
    request_header(request, line);
    version_header(version, line);
    
    //to check whether method, request and version headers meet the requirements
    
    if ((validate_method(method) == 1) || (validate_request(request) == 1) || (validate_version(version) == 1))
        return false;
        
    // seperate the absolute path and the query from request
    separate_abs_path(abs_path, request);
    separate_query(query, request);
...
//to copy the method-header from a request-line. It is at the beginning of a request-line and ends with a space.

void method_header(char* method, const char* line) {
    strcpy(method, line);
    strchr(method, ' ')[0] = '\0';
}
//to copy the request header from a request line. It is between spaces
void request_header(char* request, const char* line) {
    strcpy(request, strchr(line, ' ') + 1);
    strrchr(request, ' ')[0] = '\0';
}
//to copy the version header from the request line. It begins by the last occurance of a space and ends by the CRLF
void version_header(char* version, const char* line) {
    strcpy(version, strrchr(line, ' ') + 1);
    strstr(version, "\r\n")[0] = '\0';
}
//function to check whether the version header is valid
int validate_version(const char* version) {
    if (strcmp(version, valid_version) == 0)
        return 0;
    error(505);
    return 1;
}
//function to check whether the request header is valid
int validate_request(const char* request) {
    // to check whether the request header start with '/'
    if (request[0] != '/') {
        error(501);
        return 1;
    }
    // to check whether the request header does not contain '"'
...
//function to check whether the method header is valid
int validate_method(const char* method) {
    if (strcmp(method, valid_method) == 0)
        return 0;
    error(405);
    return 1;
}
//to seperate the query substring from the request. Query starts with '?' and ends with space ' '.
void separate_query(char* query, const char* request) {
    if (strchr(request, '?')) {
...
//to seperate the absolute path substring from the request. Absolute path contains no '?' and the questions mark seperates the absolute path from the query.
